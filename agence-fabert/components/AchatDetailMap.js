...
// Composants Leaflet dynamiques (SSR false)
...
export default function AchatDetailMap({ lat, lng, address }) {
  ...
  // Valeurs par défaut si lat/lng invalides
  const defaultPosition = [48.8566, 2.3522]; // Paris

  // Convertir lat/lng en nombres
  const position = [
    lat ? parseFloat(lat) : defaultPosition[0],
    lng ? parseFloat(lng) : defaultPosition[1],
  ];
